...
- 2026-05-08: ADR-018 既定値段階展開の **empirical PASS 後の昇格** を実施。`make_backend()` 既定値を `stub` から `claude-code` へ変更（`agent/orchestration/llm.py:321`）。`CLAUDE.md` の `agent regenerate` 運用ガード注記を解除し、Phase 2-A 受入れ §6 残課題 A-1-6 / A-6-2 / A-6-3（`vault/90_meta/metrics.md` への 16 本機械的算出値記録）を解消。テスト 202 → 203 件 PASS（`test_default_returns_claude_code` 追加 + `test_explicit_stub_returns_stub` 追加）。
