...
2. **docs/core/のドキュメントを目的別に確認する**
   ...
   **必須（常に確認）:**
   ...
   ```
   - データモデル・コンポーネント間のシーケンス・既存ユースケースを把握
   - 新機能が既存ユースケースを変更する場合、影響範囲を特定

   ```
   Read('docs/core/decisions.md')
   ```
   - 既存の設計判断（ADR）を確認
   - 新機能が既存 ADR と矛盾しないか、または新たな ADR が必要か判断
   - 「未決定事項」セクションに該当項目があれば、本タスクで確定するか確認
...
2. **design.mdの生成**
   ...
   **2.2 docs/core/との整合性確認チェックリスト:**
   ...
   | 既存ユースケース | functional-design.md | 既存のシーケンス・データモデルへの影響範囲が特定されているか |
   | 既存設計判断 | decisions.md | 既存 ADR と矛盾しないか / 新規 ADR が必要か |
